...
void sigchld_handler(int sig); //when child dies
void sigtstp_handler(int sig); //Ctrl-z pause
void sigint_handler(int sig);  //Ctrl-c terminate
...
void eval(char *cmdline) 
{
  ...
  if(!builtin_cmd(argv)){
		...
		addjob(jobs,pid, bg?BG:FG, cmdline); //Add job to job struct. Set BG/FG depending on val of bg
		if(!bg){
			waitfg(pid);
		}
		else{
